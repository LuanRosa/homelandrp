...
UpdateVehicleLabel(vehicleid, removeold)
{
	if(VehicleCreated[vehicleid] == VEHICLE_DEALERSHIP)
	{
		...
		new labeltext[128];
		format(labeltext, sizeof(labeltext), "{FF0000}%s\n{FFFFFF}ID: {FF0000}%d\n{FFFFFF}Concessionario: {FF0000}%s\n{FFFFFF}Precio: {FF0000}$%d", VehicleNames[VehicleModel[vehicleid]-400],
...
UpdateDealership(dealerid, removeold)
{
	if(DealershipCreated[dealerid])
	{
		...
		new labeltext[32];
		format(labeltext, sizeof(labeltext), "{FF0000}Spawn Vehiculo\n{FFFFFF}ID: {FF0000}%d", dealerid);
...
UpdateFuelStation(stationid, removeold)
{
	if(FuelStationCreated[stationid])
	{
		...
		new labeltext[32];
		format(labeltext, sizeof(labeltext), "{FF0000}Puesto Gasolinera\n{FFFFFF}ID: {FF0000}%d\n/gasolinera", stationid);
				...
				if(!IsBicycle(vehicleid) && Fuel[vehicleid] > 0)
				  {
					if(Fuel[vehicleid] <= 0)
					{
						...
						GameTextForPlayer(i, "~r~ Sin combustible, vaya en puesto de gasolinera y haga /gasolinera", 3000, 3);
						SendClientMessage(i, COLOR_RED, "Este vehículo se ha quedado sin combustible!");
					}
				}
			}
...
{
	if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendClientMessage(playerid, COLOR_RED, "No estas conduciendo un vehículo!");
	new vehicleid = GetPlayerVehicleID(playerid);
	if(IsTrailerAttachedToVehicle(vehicleid))
	{
		DetachTrailerFromVehicle(vehicleid);
		SendClientMessage(playerid, COLOR_WHITE, "Ya no estas remolcando");
		return 1;
	}
	new Float:x, Float:y, Float:z;
	...
	if(!closeveh) return SendClientMessage(playerid, COLOR_RED, "No estas cerca de un vehículo!");
	AttachTrailerToVehicle(closeveh, vehicleid);
	SendClientMessage(playerid, COLOR_WHITE, "Ahora esta remolcando un vehículo");
	return 1;
}
...
CMD:ejetar(playerid, params[])
{
	...
	if(!IsPlayerConnected(pid)) return SendClientMessage(playerid, COLOR_RED, "Jugador no valido!");
	new vehicleid = GetPlayerVehicleID(playerid);
	if(!IsPlayerInVehicle(pid, vehicleid)) return SendClientMessage(playerid, COLOR_RED, "El jugador no está en su vehículo!");
	RemovePlayerFromVehicle(pid);
	format(msg, sizeof(msg), "El conductor del vehículo %s (%d) lo ha expulsado", PlayerName(playerid), playerid);
	SendClientMessage(pid, COLOR_WHITE, msg);
	format(msg, sizeof(msg), "Has expulsado a %s (%d) de tu vehículo", PlayerName(pid), pid);
...
CMD:ejetarll(playerid, params[])
{
	...
	SendClientMessage(playerid, COLOR_WHITE, "Has expulsado a todos los pasajeros.");
	return 1;
}
...
CMD:clearmods(playerid, params[])
{
	...
	if(GetPlayerVehicleAccess(playerid, id) < 2)
		return SendClientMessage(playerid, COLOR_RED, "Este no es tu vehículo!");
	for(new i=0; i < sizeof(VehicleMods[]); i++)
	{
	...
	SaveVehicle(id);
	SendClientMessage(playerid, COLOR_WHITE, "Has eliminado todas las modificaciones de tu vehículo.");
	return 1;
}
...
CMD:localizarv(playerid, params[])
{
	if(TrackCar[playerid])
	{
		...
		SendClientMessage(playerid, COLOR_WHITE, "Ya no estas rastreando tu vehículo");
		return 1;
	}
	new playername[24];
	GetPlayerName(playerid, playername, sizeof(playername));
	...
	for(new i=1; i < MAX_DVEHICLES; i++)
	{
		...
		{
			found = true;
			format(info, sizeof(info), "{FF0000}%s{FFFFFF}ID: {FF0000}%d  {FFFFFF}Name: {FF0000}%s\n", info, i, VehicleNames[VehicleModel[i]-400]);
		}
	}
	if(!found) return SendClientMessage(playerid, COLOR_RED, "No tienes ningun vehículo!");
	ShowPlayerDialog(playerid, DIALOG_FINDVEHICLE, DIALOG_STYLE_LIST, "Encuentra tu vehículo", info, "Encontrar", "Cancelar");
	return 1;
}
...
CMD:mv(playerid, params[])
{
	if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendClientMessage(playerid, COLOR_RED, "No estas conduciendo un vehículo!");
	new vehicleid = GetPlayerVehicleID(playerid);
	if(IsBicycle(vehicleid)) return SendClientMessage(playerid, COLOR_RED, "No estas conduciendo un vehículo!");
	new id = GetVehicleID(vehicleid);
	if(GetPlayerVehicleAccess(playerid, id) < 1)
		return SendClientMessage(playerid, COLOR_RED, "No tienes las llaves de este vehículo!");
...
CMD:venderv(playerid, params[])
{
	...
	if(!IsPlayerConnected(pid)) return SendClientMessage(playerid, COLOR_RED, "Jugador no valido!");
	if(GetPlayerVehicleAccess(playerid, id) < 2)
		return SendClientMessage(playerid, COLOR_RED, "Usted no es el dueno de este vehículo!");
	if(price < 1) return SendClientMessage(playerid, COLOR_RED, "Precio no valido!");
	if(!PlayerToPlayer(playerid, pid, 10.0)) return SendClientMessage(playerid, COLOR_RED, "El jugador esta demasiado lejos!");
	...
	format(msg, sizeof(msg), "Ha ofrecido a %s (%d) comprar su vehículo por $%d", PlayerName(pid), pid, price);
	SendClientMessage(playerid, COLOR_WHITE, msg);
	return 1;
}
...
CMD:darllaves(playerid, params[])
{
	...
	if(!IsPlayerConnected(pid)) return SendClientMessage(playerid, COLOR_RED, "Jugador no valido!");
	if(!IsValidVehicle(id)) return SendClientMessage(playerid, COLOR_RED, "ID de vehículo no valido!");
	if(GetPlayerVehicleAccess(playerid, id) < 2)
		return SendClientMessage(playerid, COLOR_RED, "Usted no es el propietario de este vehículo!");
	if(!PlayerToPlayer(playerid, pid, 10.0)) return SendClientMessage(playerid, COLOR_RED, "El jugador esta demasiado lejos!");
	SetPVarInt(pid, "CarKeys", id);
	format(msg, sizeof(msg), "Le has dado las llaves de tu coche a %s (%d)", PlayerName(pid), pid);
	SendClientMessage(playerid, COLOR_WHITE, msg);
	format(msg, sizeof(msg), "%s (%d) te ha dado las llaves del auto", PlayerName(playerid), playerid);
	SendClientMessage(pid, COLOR_WHITE, msg);
	return 1;
}
...
CMD:seguro(playerid, params[])
{
	...
	if(!vehicleid) return SendClientMessage(playerid, COLOR_RED, "No estas cerca de un vehículo!");
	new id = GetVehicleID(vehicleid);
	if(!IsValidVehicle(id)) return SendClientMessage(playerid, COLOR_RED, "No tienes las llaves de este vehículo!");
	if(GetPlayerVehicleAccess(playerid, id) < 2)
		return SendClientMessage(playerid, COLOR_RED, "No tienes las llaves de este vehículo!");
	new engine, lights, alarm, doors, bonnet, boot, objective;
	GetVehicleParamsEx(vehicleid, engine, lights, alarm, doors, bonnet, boot, objective);
	if(doors == 1)
	{
		doors = 0;
		VehicleLock[id] = 0;
		GameTextForPlayer(playerid, "~g~puertas desbloqueadas", 3000, 6);
	}
	else
	{
		doors = 1;
		VehicleLock[id] = 1;
		GameTextForPlayer(playerid, "~r~puertas cerradas", 3000, 6);
	}
...
CMD:alarmev(playerid, params[])
{
	...
	if(VehicleSecurity[vehicleid] == 0)
	{
		...
		VehicleAlarm[id] = 1;
		GameTextForPlayer(playerid, "~g~alarma encendida", 3000, 6);
	}
	else
	{
		ToggleAlarm(vehicleid, VEHICLE_PARAMS_OFF);
		VehicleSecurity[vehicleid] = 0;
		VehicleAlarm[id] = 0;
		GameTextForPlayer(playerid, "~r~alarma apagada", 3000, 6);
	}
	SaveVehicle(id);
	return 1;
}
...
CMD:maletero(playerid, params[])
{
	...
	if(!vehicleid || IsBicycle(vehicleid) || IsPlayerInAnyVehicle(playerid))
		return SendClientMessage(playerid, COLOR_RED, "No estas cerca de un vehículo!");
	new id = GetVehicleID(vehicleid);
	if(!IsValidVehicle(id)) return SendClientMessage(playerid, COLOR_RED, "No tienes las llaves de este vehículo!");
	if(GetPlayerVehicleAccess(playerid, id) < 2)
		return SendClientMessage(playerid, COLOR_RED, "No tienes las llaves de este vehículo!");
...
CMD:gasolinera(playerid, params[])
{
	for(new i=1; i < MAX_FUEL_STATIONS; i++)
	{
		if(FuelStationCreated[i])
	...
	SendClientMessage(playerid, COLOR_RED, "No estas en un puesto de combustible!");
	return 1;
}
...
CMD:rtc(playerid, params[])
{
	...
	GetPlayerName(playerid, sendername, sizeof(sendername));
    if(!strcmp(sendername,"Luan_Rosa", false))
	if(!IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, COLOR_RED, "No estas en un vehículo!");
	SetVehicleToRespawn(GetPlayerVehicleID(playerid));
	SendClientMessage(playerid, COLOR_WHITE, "Vehículo reaparecido");
	return 1;
}
...
CMD:rac(playerid, params[])
{
	...
	new msg[128];
	format(msg, sizeof(msg), "El administrador %s (%d) ha reaparecido todos los vehículos sin usar", PlayerName(playerid), playerid);
...
CMD:setfuel(playerid, params[])
{
	...
	format(msg, sizeof(msg), "Ha establecido el combustible de su vehículo en %d", amount);
	SendClientMessage(playerid, COLOR_WHITE, msg);
	return 1;
}
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	if(!ispassenger)
	{
		...
		{
			new msg[128];
			format(msg, sizeof(msg), "Este vehículo pertenece a %s", VehicleOwner[id]);
...
ShowDialog(playerid, dialogid)
{
	switch(dialogid)
	{
		case DIALOG_VEHICLE:
		{
			...
			format(caption, sizeof(caption), "Vehículo %d", vehicleid);
			strcat(info, "Motor\nLuces\nCapó\nMaletero", sizeof(info));
			strcat(info, "\nFill Tank", sizeof(info));
			if(GetPlayerVehicleAccess(playerid, vehicleid) >= 2)
			{
				new value = VehicleValue[vehicleid]/2;
				format(info, sizeof(info), "%s\nVender vehículo ($%d)\nEstacionar vehículo\nEditar Placa", info, value);
			}
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_LIST, caption, info, "Seleccionar", "Cancelar");
		}
		case DIALOG_VEHICLE_BUY:
		{
			new vehicleid = GetPVarInt(playerid, "DialogValue1");
			new caption[32], info[256];
			format(caption, sizeof(caption), "Vehiculo %d", vehicleid);
			format(info, sizeof(info), "Este vehículo esta a la venta ($%d)\n-", VehicleValue[vehicleid]);
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_MSGBOX, caption, info, "Comprar", "Cancelar");
		}
		case DIALOG_VEHICLE_SELL:
		{
			...
			new info[256];
			format(info, sizeof(info), "%s (%d) quiere venderte un %s por $%d.", PlayerName(targetid), targetid,
				VehicleNames[VehicleModel[id]-400], price);
			strcat(info, "\n\n Te gustaría comprar?", sizeof(info));
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_MSGBOX, "Comprar Vehiculo", info, "Si", "No");
		}
		case DIALOG_TRUNK:
		{
			new vehicleid = GetPVarInt(playerid, "DialogValue1");
			...
			for(new i=0; i < sizeof(VehicleTrunk[]); i++)
			{
				...
				{
					format(info, sizeof(info), "%s%d. Vacío\n", info, i+1);
				}
			}
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_LIST, "Maletero", info, "Seleccionar", "Cancelar");
		}
		case DIALOG_TRUNK_ACTION:
		{
			new info[128];
			strcat(info, "Poner en el maletero\nTomar del maletero", sizeof(info));
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_LIST, "Maletero", info, "Seleccionar", "Cancelar");
		}
		case DIALOG_VEHICLE_PLATE:
		{
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, "Editar Placa", "Introducir nueva placa:", "Cambiar", "Atras");
		}
		case DIALOG_FUEL:
		{
			new info[128];
			strcat(info, "Repostar Vehículo ($" #FUEL_PRICE ")\nComprar bidon de gasolina ($" #GAS_CAN_PRICE ")", sizeof(info));
		...
		case DIALOG_EDITVEHICLE:
		{
			...
			strcat(info, "\n5. Eliminar vehículo\n6. Estacionar vehículo\n7. Ir al vehículo", sizeof(info));
			strcat(info, "\n\nEnter: [nr] [value1] [value2]", sizeof(info));
			ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, caption, info, "OK", "Cancelar");
		}
	}
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
		...
		if(response)
		{
			switch(listitem)
			{
				case 0:
				{
					...
					{
						ShowErrorDialog(playerid, "Este vehículo se ha quedado sin combustible!");
						return 1;
					}
				...
				case 4:
				{
					...
					{
						ShowErrorDialog(playerid, "No tienes una lata de gasolina!");
						return 1;
					}
					new vehicleid = GetPlayerVehicleID(playerid);
					...
					SendClientMessage(playerid, COLOR_WHITE, "Ha llenado el tanque de combustible con 20 combustible");
				}
				case 5:
				{
					...
					if(GetPlayerVehicleAccess(playerid, id) < 2)
					{
						ShowErrorDialog(playerid, "Usted no es el dueno de este vehículo!");
						return 1;
					}
					new msg[128];
					VehicleCreated[id] = 0;
					...
					format(msg, sizeof(msg), "Has vendido tu vehículo por $%d", money);
					SendClientMessage(playerid, COLOR_WHITE, msg);
				...
				case 6:
				{
					...
					SendClientMessage(playerid, COLOR_WHITE, "Ha aparcado este vehículo aquí");
	...
	if(dialogid == DIALOG_VEHICLE_BUY)
	{
		if(response)
		{
			...
			{
				ShowErrorDialog(playerid, "No puedes comprar mas vehículos! máx.:" #MAX_PLAYER_VEHICLES );
				return 1;
			}
			new id = GetPVarInt(playerid, "DialogValue1");
			if(GetPlayerMoney(playerid) < VehicleValue[id])
			{
				ShowErrorDialog(playerid, "No tienes suficiente dinero para comprar este vehículo!");
				return 1;
			}
			new freeid = GetFreeVehicleID();
			if(!freeid)
			{
				ShowErrorDialog(playerid, "El concesionario de vehículos esta agotado!");
				return 1;
			}
			...
			SaveVehicle(freeid);
			new msg[128];
			format(msg, sizeof(msg), "Has comprado este vehículo por $%d", VehicleValue[id]);
			SendClientMessage(playerid, COLOR_WHITE, msg);
		}
		else
		{
	...
	if(dialogid == DIALOG_VEHICLE_SELL)
	{
		if(response)
		{
			if(GetPlayerVehicles(playerid) >= MAX_PLAYER_VEHICLES)
			{
				ShowErrorDialog(playerid, "No puedes comprar más vehículos! máx.:" #MAX_PLAYER_VEHICLES );
				return 1;
			}
			new targetid = GetPVarInt(playerid, "DialogValue1");
			new id = GetPVarInt(playerid, "DialogValue2");
			...
			SaveVehicle(id);
			format(msg, sizeof(msg), "Has comprado este vehículo por $%d", price);
			SendClientMessage(playerid, COLOR_WHITE, msg);
			format(msg, sizeof(msg), "%s (%d) acepto su oferta y compro el vehículo", PlayerName(playerid), playerid);
		...
		else
		{
			new targetid = GetPVarInt(playerid, "DialogValue1");
			new msg[128];
			format(msg, sizeof(msg), " %s (%d) rechazO su oferta", PlayerName(playerid), playerid);
			SendClientMessage(targetid, COLOR_WHITE, msg);
		}
		return 1;
	}
	if(dialogid == DIALOG_FINDVEHICLE)
	{
		if(response)
		{
			...
			if(IsValidVehicle(id))
			{
				...
				SendClientMessage(playerid, COLOR_WHITE, "La ubicacion de su vehículo se muestra en su minimapa");
			}
		}
		return 1;
	}
	...
	if(dialogid == DIALOG_TRUNK_ACTION)
	{
		if(response)
		{
			...
			case 0:
			{
				...
				if(weaponid == 0)
				{
					ShowErrorDialog(playerid, "No tienes un arma en tus manos!");
					return 1;
				}
			...
			case 1:
			{
				...
				{
					ShowErrorDialog(playerid, "Esta ranura esta vacía!");
					return 1;
				}
	...
	if(dialogid == DIALOG_VEHICLE_PLATE)
	{
		if(response)
		{
			...
			new msg[128];
			format(msg, sizeof(msg), "Ha cambiado la placa del vehículo a %s", inputtext);
			SendClientMessage(playerid, COLOR_WHITE, msg);
		}
	...
	if(dialogid == DIALOG_FUEL)
	{
		if(response)
		{
			...
			case 0:
			{
				if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
				{
					ShowErrorDialog(playerid, "No estas conduciendo un vehículo!");
					return 1;
				}
				new vehicleid = GetPlayerVehicleID(playerid);
				if(IsBicycle(vehicleid))
				{
					ShowErrorDialog(playerid, "Nuestro vehículo no tiene tanque de combustible!");
					return 1;
				}
				if(Fuel[vehicleid] >= 100.0)
				{
					ShowErrorDialog(playerid, "El tanque de combustible de su vehículo está lleno!");
					return 1;
				}
				if(GetPlayerMoney(playerid) < FUEL_PRICE)
				{
					ShowErrorDialog(playerid, "No tienes suficiente dinero!");
					return 1;
				}
				RefuelTime[playerid] = 5;
				SetPVarFloat(playerid, "Fuel", Fuel[vehicleid]);
				GameTextForPlayer(playerid, "~w~repostaje...", 2000, 3);
			}
			case 1:
			{
				if(GetPVarInt(playerid, "GasCan"))
				{
					ShowErrorDialog(playerid, "Ya tienes una lata de gasolina!");
					return 1;
				}
				...
				SendClientMessage(playerid, COLOR_WHITE, "Has comprado una lata de gasolina por $" #GAS_CAN_PRICE );
			}
			}
		}
		return 1;
	...
	if(dialogid == DIALOG_EDITVEHICLE)
	{
		if(response)
		{
			...
			case 2:
			{
				...
				{
					ShowErrorDialog(playerid, "Modelo de vehículo no valido!");
					return 1;
				}
			...
			case 5:
			{
				...
				SaveVehicle(id);
				new msg[128];
				format(msg, sizeof(msg), "Ha eliminado la identificación del vehículo %d", id);
				SendClientMessage(playerid, COLOR_WHITE, msg);
			}
			case 6:
			{
				if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
				{
					ShowErrorDialog(playerid, "Usted no está conduciendo el vehículo!");
					return 1;
				}
			...
			case 7:
			{
				...
				new msg[128];
				format(msg, sizeof(msg), "Te has teletransportado a vehículo %d", id);
				SendClientMessage(playerid, COLOR_WHITE, msg);
			}
			}
		}
